var methodsToMock = require('./methodsToMock');
...
var Mock = {
    ...
    boostrap: function(){
        ...
        Mock.NewtonMock = {};
        for(var key in methodsToMock){
        // for(var i=0; i < methodsToMock.length; i++){
            // var methodName = methodsToMock[i];
            Mock.NewtonMock[key] = function(callback){
                Mock.calls.push(key);
                if(methodsToMock[key].callback && typeof (callback) === 'function'){ callback.call(); }
                if(key === 'isUserLogged'){
                    return Mock.logged;
                } else if(methodsToMock[key].return) {
                    return this;
                }
                spyOn(Mock.NewtonMock, key).and.callThrough();
            };
        }
